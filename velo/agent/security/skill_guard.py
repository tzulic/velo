...
_THREAT_PATTERNS: list[tuple[str, str, str]] = [
    ...
    (r"curl\s+[^\n]*\$\{?\w*(KEY|TOKEN|SECRET|PASSWORD|API)", "exfiltration", _CRITICAL),
    (r"wget\s+[^\n]*\$\{?\w*(KEY|TOKEN|SECRET|PASSWORD|API)", "exfiltration", _CRITICAL),
    (r"cat\s+[^\n]*(\.env|credentials|\.netrc|\.pgpass)", "exfiltration", _CRITICAL),
    (r"\bns(lookup|update)\s+[^\n]*\$", "exfiltration_dns", _CRITICAL),
    (r"base64\s+.*\|\s*(curl|wget|nc)", "exfiltration_encoded", _CRITICAL),
    # --- Prompt injection (high) ---
    (r"ignore\s+(previous|all|above)\s+instructions", "prompt_injection", _HIGH),
    (r"you\s+are\s+now\s+", "prompt_injection", _HIGH),
    (r"system\s+prompt\s+override", "prompt_injection", _HIGH),
    (r"disregard\s+(your|all|any)\s+(instructions|rules)", "prompt_injection", _HIGH),
...
_COMPILED_THREATS = [
    (re.compile(pat, re.IGNORECASE), cat, sev) for pat, cat, sev in _THREAT_PATTERNS
]
...
def scan_skill(content: str, source: str = "community") -> SkillVerdict:
