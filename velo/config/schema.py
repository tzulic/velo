...
class A2APeerConfig(Base):
    """A peer A2A agent that this velo can delegate tasks to."""
...
class MCPServerConfig(Base):
    ...
    defer_tools: bool = True  # register MCP tools as deferred (loaded on-demand via search_tools)
...
class Config(BaseSettings):
    ...
    def get_api_base(self, model: str | None = None) -> str | None:
        ...
        if p and p.api_base:
            return p.api_base
        # Resolve default_api_base for ALL providers (gateways, DeepSeek, Groq, etc.)
        if name:
    ...
    model_config = ConfigDict(env_prefix="VELO_", env_nested_delimiter="__")
