...
chmod 600 ~/.velo/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.velo/config.json` with file permissions set to `0600`
...
- ✅ Run velo with a dedicated user account
...
- Keep authentication data in `~/.velo/whatsapp-auth` secure (mode 0700)
...
- Subscribe to security advisories for velo and its dependencies
...
2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash velo
   sudo -u velo velo gateway
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.velo
   chmod 600 ~/.velo/config.json
   chmod 700 ~/.velo/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.velo/logs/velo.log
   ```
...
- **Chat history is stored locally** - protect the `~/.velo` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.velo/logs/velo.log
   ```
